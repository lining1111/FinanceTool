...
type BalanceSheet struct {
	...
	F005N float64 `excel:"固定资产"`           //固定资产	numeric
	F006N float64 `excel:"投资"`             //投资	numeric
	F007N float64 `excel:"其他非流动资产"`        //其他非流动资产	numeric
	F008N float64 `excel:"非流动资产总值"`        //非流动资产总值	numeric
	F009N float64 `excel:"流动资产总值"`         //流动资产总值	numeric
	F010N float64 `excel:"其中：存货"`          //其中：存货	numeric
	F011N float64 `excel:"其中：现金及银行结存"`     //其中：现金及银行结存	numeric
	F012N float64 `excel:"总资产"`            //总资产	numeric
	F013N float64 `excel:"短期债项"`           //短期债项	numeric
	F014N float64 `excel:"流动负债总值"`         //流动负债总值	numeric
	F015N float64 `excel:"长期债项"`           //长期债项	numeric
	F016N float64 `excel:"其他非流动负债及少数股东权益"` //其他非流动负债及少数股东权益	numeric
	F017N float64 `excel:"总债项"`            //总债项	numeric
	F018N float64 `excel:"股本"`             //股本	numeric
	F019N float64 `excel:"储备"`             //储备	numeric
	F020N float64 `excel:"股东权益"`           //股东权益	numeric
	F021V string  `excel:"核数师意见"`          //核数师意见	varchar
	F022N float64 `excel:"营运资金"`           //营运资金	numeric
	F023V string  `excel:"报表类型编码"`         //报表类型编码	char		其中1代表半年报，2代表年报
