...
## 20230417 间隔一段时间回来后，
    程序界应该是有许多基础库可拿来直接应用，比如QuantLib，但是这个库只有c++版本，以及c++延伸出来的python、csharp、ruby等，没有go的版本
    从上次管理这个程序到现在回来，已经有一个月了，期间大致的略读了《金融工程》，正在读的是《数理金融》
    得到的感悟是，先应该先从分析一个具体的业务开始(比如种地，制造一种商品等)，
    1、先将复杂问题简单化处理：
        1.1 分析一项业务所需的基本可量化指标，如：固定成本、可变成本、可变成本中的，可以分解出很多与商品单位相关的元素，比如(原材料、人工等)
        1.2 按盈亏均衡理论，得到现实的盈亏平衡点，这里可以加入利率，即货币的时间价值
    2、简单处理后，将问题逐步复杂化：
        2.1 通过理论计算值，对比同行业企业的现状，验证理论的实践性
        2.2 将偏差分为正向和负向两种，即 现状大于理论值，现状小于理论值。分别从实际出发，列出除1.1所列元素外的生产元素。
    3、简单业务金融工程处理
        3.1、将生产元素与实际业务的历史数据做表，分析相关性，即、正负相关
        3.2、通过相关性处理，的到商品收益的金融工程化处理公式。
    4、通过3的到的公式，进行对冲处理
        4.1、通过金融工程化处理后，做对冲处理，选择合适的策略
## 20230418 使用的线性代数库
    go主要是使用gonum
    线性代数的意义在于解方程组，方程组的意义在于，变量其实是系数，系数呢是变量(这里的变量就是实际观察数据)，相关性是选择数据的过程
    这句话正过来说的意思就是，从大数据内，通过相关性筛选相关数据，然后建立方程组，通过线性代数求解数据的系数，建立实际过程的公式化处理
