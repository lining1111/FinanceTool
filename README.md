...
## 20230314
    ...
    综上所述、投资初期和后续执行按步骤应该是
        ...
        2、将每一个小目标分解成若干实际的财务指标,需要输入特点的评价系统名称
            ...
            这里应该先判断目标的可行性。即按行业分类，获取当前行业内的各个股票在近期的横向分析。
            在时间上从近期往上推4个季度的财报指标。如先获取最近一期的这个行业的所有股票的财报指标，通过它们的数组做正态分布
            对比所设目标在正态分布内的情况。然后依次推4个季度的。
...
## 20230316 从投资初期选股，看选择比努力重要的原因

    初期设定目标，并根据这个目标来选取投资对象，选择分为以下几个步骤：
    1、首先应该判断目标的实际可行性
        首选根据选的的指标类型，根据行业在各个时间点判断该指标的横向情况。
        例：市盈率PE一般先简单作为股票的盈利。则
        1.1 以PE为搜索当量，得到当前所有股票的在该时间点的记录
            {PE,[]{scode string, eval string,val float64}} //PE 市盈率 scode股票代码 eval行业分类 val 数值
        1.2 将数组，按行业进行二次分组 
            []{eval string, {scode string, val float64}}
        1.3 将行业分组后的记录
            先按纵向分析，算出 
                max最大值 min最小值 avg平均值  平均值，可以看出自己定的目标和平均值以及最高值、最低值的对比情况
                差值数组[] 绘制柱状图，可以看到当期这个行业内各股的差异
                AverageDeviation平均差值 这个值可以留做和当期其他行业进行对比
                Variance 方差  这个值可以留做和当期其他行业进行对比
                StdDeviation 标准差 这个值可以留做和当期其他行业进行对比
                前面的数据集合[最大值、最小值、平均值、差值数组]初步判断自我期望和实际期望的差距
                后面的数据集合[平均差值、标准差]在行业间进行判断，确定各行业当期的实际期望的稳定性
                ===这一步的作用，就是选择各个行业赛道，结合自身知识储备选择行业较为稳定的大类。
            再按横向分析，即当期行业内，各支股票的分布情况，正态分布情况
                在行业内有这样的数组 []{scode string，val float64}
                先按值从小到大排序，得到有序数组
                还是以PE为例，以0.01为步进值。从最小值到最大值，分别计算数量，如
                0-1之间，会出现100+1个值，得到这样的数组[]{PE float64, count int64}
                这样的数组就可以按照正态分布分析。
                (原因就是现实的生活就是，赚得少的，慢慢的死掉了，赚得多的毕竟是少数，大多数的都集中在即撑不死，也饿不死的地步)
                正态分布的分析，也是风险和收益的代表。这里就应该选择PE值高但又不过分高的。
            经过上面的两步，的到的是行业稳定内，收益较高的待选股票数组。
            选择完待选股票后，就要对单个股票的历史数据做分析。
    选择的宗旨，就是在近期一个经营周期内，选择行业稳定、收益较高的，待选数组。然后在待选数组内，通过历史记录选择风险少的。
